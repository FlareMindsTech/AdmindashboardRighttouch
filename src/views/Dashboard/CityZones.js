...
export default function CityZones() {
  ...
  // Direct District / City Toggle
  const handleToggleDistrictAvailabilityDirect = async (dist) => {
    if (!selectedServiceDetail?.service) return;
    const serviceId = selectedServiceDetail.service.serviceId || selectedServiceDetail.service._id;
    const districtId = dist.districtId || dist._id;
    ...
    const childZonesFromDist = Array.isArray(dist.zones) ? dist.zones : [];
    const childZonesFromState = (zones || []).filter(
      (z) => String(z.operationalCityId || z.districtId) === String(districtId)
    );
    const allChildZones = childZonesFromDist.length > 0 ? childZonesFromDist : childZonesFromState;
    const zoneIds = allChildZones.map((z) => z.zoneId || z._id || z.id).filter(Boolean);
    ...
    setSelectedServiceDetail((prev) => {
      ...
      return {
        ...
        districts: prev.districts.map((d) => {
          if (String(d.districtId || d._id) === String(districtId)) {
            const updatedZones = (d.zones || allChildZones || []).map((z) => ({
              ...z,
              status: nextStatus,
              enabled: nextEnabled,
              active: nextEnabled,
      ...
      if (nextEnabled) {
        ...
        if (zoneIds.length > 0) {
          promises.push(
            bulkToggleZoneAvailability({
              ...
              enabled: true,
              status: "ENABLED",
            }).catch((err) => console.warn("bulkToggleZoneAvailability enable warning:", err.message))
          );
          ...
          promises.push(
            ...
            ).catch((err) => console.warn("bulkCreateZoneMappings enable warning:", err.message))
          );
        }
        ...
        promises.push(
          createServiceAvailability({
            ...
            console.warn("createServiceAvailability enable warning:", err.message);
            if (dist.availabilityId || dist.serviceAvailabilityId) {
              await updateServiceAvailability(dist.availabilityId || dist.serviceAvailabilityId, {
                status: "ENABLED",
                scope: "DISTRICT",
              }).catch(() => {});
            } else {
        ...
        promises.push(
          toggleZoneAvailability({
            ...
            enabled: true,
            status: "ENABLED",
          }).catch((err) => console.warn("toggleZoneAvailability enable warning:", err.message))
        );
        ...
        if (zoneIds.length > 0) {
          promises.push(
            bulkToggleZoneAvailability({
              ...
              enabled: false,
              status: "DISABLED",
            }).catch((err) => console.warn("bulkToggleZoneAvailability disable warning:", err.message))
          );
          ...
          promises.push(
            ...
            ).catch((err) => console.warn("bulkCreateZoneMappings disable warning:", err.message))
          );
        }
        ...
        promises.push(
          clearDistrictZoneAvailability({
            serviceId,
            districtId,
          }).catch((err) => console.warn("clearDistrictZoneAvailability warning:", err.message))
        );
        ...
        promises.push(
          createServiceAvailability({
            ...
            console.warn("createServiceAvailability disable warning:", err.message);
            if (dist.availabilityId || dist.serviceAvailabilityId) {
              await updateServiceAvailability(dist.availabilityId || dist.serviceAvailabilityId, {
                status: "DISABLED",
                scope: "DISTRICT",
        ...
        promises.push(
          toggleZoneAvailability({
            ...
            enabled: false,
            status: "DISABLED",
          }).catch((err) => console.warn("toggleZoneAvailability disable warning:", err.message))
        );
        ...
        districts: prev.districts.map((d) => {
          if (String(d.districtId || d._id) === String(districtId)) {
            const updatedZones = (d.zones || []).map((z) =>
              String(z.zoneId || z._id || z.id) === String(zoneId)
    ...
    try {
      const promises = [
        toggleZoneAvailability({
          ...
          enabled: nextEnabled,
          status: nextStatus,
        }).catch((err) => console.warn("toggleZoneAvailability error:", err.message)),
        bulkCreateZoneMappings([
          {
            zoneId,
            serviceId,
        ...
        ]).catch((err) => console.warn("bulkCreateZoneMappings error:", err.message)),
      ];
  ...
  const handleTriggerSingleZoneToggle = (zone, currentStatus, districtObj) => {
    ...
    setConfirmConfig({
      ...
      payload: {
        serviceId: selectedServiceDetail?.service?.serviceId,
        districtId: districtObj?.districtId,
        cityZoneId: zone.zoneId,
        status: nextStatus,
      },
    });
  ...
  const handleExecuteConfirmedAction = async () => {
    ...
    try {
      ...
      if (selectedServiceDetail?.service?.serviceId) {
        await reloadServiceDetail(selectedServiceDetail.service.serviceId);
      }
      await fetchMatrixData();
    } catch (err) {
  ...
  const filteredZones = useMemo(() => {
    ...
    return zones.filter((zone) => {
      ...
      if (selectedDistrictFilter !== "all") {
        const distId = zone.operationalCityId?._id || zone.operationalCityId;
        return matchesSearch && distId === selectedDistrictFilter;
      }
      return matchesSearch;
    });
  ...
  const handleSaveServiceRadius = async () => {
    ...
    try {
      setIsSubmitting(true);
      await updateTechnician({ id: selectedTechForPermission._id, serviceRadiusKm: Number(techServiceRadius) || 10 });
      toast({
  ...
  const handleOpenEditZone = (zone) => {
    ...
    if (zone.polygon && typeof zone.polygon === "object") {
      polygonStr = JSON.stringify(zone.polygon, null, 2);
    }

    setFormData({
      operationalCityId: zone.operationalCityId?._id || zone.operationalCityId || "",
      name: zone.name || "",
      zoneCode: zone.zoneCode || "",
      active: zone.active !== false,
      polygonCoordinates: polygonStr,
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmitZone = async () => {
    ...
    if (formData.polygonCoordinates.trim()) {
      try {
        const parsed = JSON.parse(formData.polygonCoordinates);
        if (parsed && parsed.type && Array.isArray(parsed.coordinates)) {
          polygon = { type: parsed.type, coordinates: parsed.coordinates };
        }
      } catch {
    ...
    try {
      setIsSubmitting(true);
      const payload = {
        operationalCityId: formData.operationalCityId,
      ...
      if (isEditing && selectedZone) {
        await updateZone(selectedZone._id, payload);
        toast({ title: "Zone Updated", description: `"${formData.name}" updated successfully.`, status: "success", duration: 3000, isClosable: true });
      } else {
        await createZone(payload);
        toast({ title: "Zone Created", description: `"${formData.name}" created successfully.`, status: "success", duration: 3000, isClosable: true });
      }
  ...
  const handleRunDispatchDebug = async () => {
    ...
    try {
      ...
      const res = await getDispatchDebug(debugDistrictId, debugServiceId || undefined);
